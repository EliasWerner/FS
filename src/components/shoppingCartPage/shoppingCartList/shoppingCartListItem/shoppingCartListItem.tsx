...
export const ShoppingCartListItem = (/* props: IShoppingCartListItemProps */) => {
  const [listItem, setListItem] = React.useState<IShoppingCartItem | null>(null);

  return (
    <div className="shopping-cart-list-item">
      <div className="shopping-cart-list-item-image">
        <img
          src={listItem?.furniture.ImageUrl}
          alt={listItem?.furniture.Title}
        />
      </div>
      <div className="shopping-cart-list-item-title">
        <h4>{listItem?.furniture.Title}</h4>
        <h5>{listItem?.furniture.Price} руб.</h5>
      </div>
